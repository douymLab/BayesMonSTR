            ...
            for read in bamfile.fetch(chrom, seg_locus.start, seg_locus.end):
                ...
                if per_read_feature.is_usable:
                    ...
                    if readsegger.is_usable:
                        if read.has_tag("HP"):
                            HP_tag_dict[read.query_name] = str(
                                read.get_tag("HP")
                            )
                        else:
                            HP_tag_dict[read.query_name] = "NA"
                        if read.has_tag("PS"):
                            PS_tag_dict[read.query_name] = str(
                                read.get_tag("PS")
                            )
                        else:
                            PS_tag_dict[read.query_name] = "NA"
                        spanning_reads_name.append(
                            readsegger.pysam_read.query_name
                        )
                        spanning_pysam_read.append(readsegger.pysam_read)
                        if config_params.ALLELE_EXTRACT["SegMethods"] == "HMM":
                            alleles_STR_length_list.append(
                                readsegger.STR_length
                            )
                            read_str_hap_accuracy = tuple(
                                [
                            ...
                            if LIKELIHOOD_MODE == "length-based":
                                alleles_depth_dict[readsegger.STR_length] = (
                                    alleles_depth_dict.get(
                                        readsegger.STR_length, 0
                                    )
                                    + 1
                                )
                                alleles_pysam_read_dict.setdefault(
                                    readsegger.STR_length, []
                                ).append(read)
                                alleles_reads_accuracy_dict.setdefault(
                                    readsegger.STR_length, []
                                ).append(read_str_hap_accuracy)
                                reads_list.append(readsegger.STR_length)
                                spanning_reads_dict[
                                    readsegger.pysam_read.query_name
                                ] = (
                                    readsegger.STR_length
                                )  # If the same query_name, that's PE reads spanning the STR the same time, we just consider one read.
                                spanning_reads_accuracy_dict[
                                    readsegger.pysam_read.query_name
                                ] = None
                                # allele_read_name_dict.setdefault(
                                #     readsegger.STR_length, []
                                # ).append(readsegger.pysam_read.query_name)
                                if read.has_tag("HP"):
                                    pacbio_phasing_allele_list.append(
                                        readsegger.STR_length
                                    )
                            else:
                                if ADD_FLANKING_PROB:
                                    read_str_allele = tuple(
                                        [
                                ...
                                #         read_str_allele, []
                                #     ).append(readsegger.pysam_read.query_name)
                                alleles_depth_dict[read_str_allele] = (
                                    alleles_depth_dict.get(read_str_allele, 0)
                                    + 1
                                )
                                alleles_pysam_read_dict.setdefault(
                                    read_str_allele, []
                                ...
                                reads_list.append(read_str_allele)
                                spanning_reads_dict[
                                    readsegger.pysam_read.query_name
                                ] = read_str_allele
                                spanning_reads_accuracy_dict[
                            ...
                            if LIKELIHOOD_MODE == "length-based":
                                alleles_depth_dict[
                                    ...
                                    alleles_depth_dict.get(
                                        readsegger.read_allele_length, 0
                                    )
                                    + 1
                                )
                                alleles_pysam_read_dict.setdefault(
