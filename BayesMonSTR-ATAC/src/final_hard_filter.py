...
def hard_filter_mode(row):
    ...
    if muttype == "homhet":
        if (mutant_pop_af > POP_AF_MAX) or (
...
def run_final_hard_filter(
    ...
    mode='both',
    het_no_filter=True,
    save_sort_features=True,
    save_prediction_features=True
):
    germ_mut = [
        "overall_mean_mapQ",
        "germ_mut_diff_mapQ",
    ...
    # print(GIAB_fp["mut_seq"],GIAB_fp["source_seq"])
    ...
    if mode == "all":
        mosaic_df_select_rf = GIAB_fp.reindex(columns=Output_RF_features)
    else:
        hard_filter_result = GIAB_fp.apply(hard_filter_mode, axis=1)
        GIAB_fp = pd.concat([GIAB_fp, hard_filter_result], axis=1)
        ...
        if mis_or_indel == "indel":
            if indel_model is not None:
                ...
                indel_model_features_order_for_rf_name[3] = "observed_mosaic_allele_vaf_single_locus"
                ...
                if not indel_df.empty:
                    ...
                    classes = all_depth_indel_model.named_steps["classifier"].classes_
                    for i, cls in enumerate(classes):
                        indel_df[cls] = indel_prediction_prob[:, i]
                    indel_df["mutation_type"] = "InDel"
                    if save_prediction_features:
                        indel_df.to_csv(".".join(output_file.split(".")[:-1]) + ".indel.csv", header=True, index=True)
                    mosaic_df_select_rf = indel_df[Output_RF_features]
                else:
                    mosaic_df_select_rf = indel_df.reindex(columns=Output_RF_features)
            else:
                print("Warning: Can not find indel model.")
                mosaic_df_select_rf = GIAB_fp.reindex(columns=Output_RF_features)
            ...
            if mismatch_model is not None:
                ...
                if not mis_df.empty:
                    ...
                    classes = all_depth_mis_model.named_steps["classifier"].classes_
                    for i, cls in enumerate(classes):
                        mis_df[cls] = mis_prediction_prob[:, i]
                    mis_df["mutation_type"] = "Mismatch"
                    if save_prediction_features:
                        mis_df.to_csv(".".join(output_file.split(".")[:-1]) + ".mis.csv", header=True, index=True)
                    mosaic_df_select_rf = mis_df[Output_RF_features]
                else:
                    mosaic_df_select_rf = mis_df.reindex(columns=Output_RF_features)
            else:
                print("Warning: Can not find mismatch model.")
                mosaic_df_select_rf = GIAB_fp.reindex(columns=Output_RF_features)
            ...
            if indel_model is not None:
                ...
                if not indel_df.empty:
                    ...
                    if save_prediction_features:
                        indel_df.to_csv(".".join(output_file.split(".")[:-1]) + ".indel.csv", header=True, index=True)
                    rf_parts.append(indel_df[Output_RF_features])
            ...
            if mismatch_model is not None:
                ...
                if not mis_df.empty:
                    ...
                    if save_prediction_features:
                        mis_df.to_csv(".".join(output_file.split(".")[:-1]) + ".mis.csv", header=True, index=True)
                    rf_parts.append(mis_df[Output_RF_features])


            if rf_parts:
                mosaic_df_select_rf = pd.concat(rf_parts)
            else:
                mosaic_df_select_rf = GIAB_fp.reindex(columns=Output_RF_features)
        else:
            raise ValueError(f"Invalid mis_or_indel: {mis_or_indel}")
    ...
    #     population_panel, reference_genome, final_mosaic_merge_df
    # )

    pysam_fasta = pysam.FastaFile(reference_genome)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Final hard filter")
    ...
    parser.add_argument("-mm", "--mismatch_model", 
                        help="Mismatch model file (optional if mode is 'all')", 
                        required=False,
                        type=str,
                        default=None) 
    parser.add_argument("-mid", "--indel_model", 
                        help="Indel model file (optional if mode is 'all')", 
                        required=False,
                        type=str,
                        default=None)
    parser.add_argument(
        "-r", "--reference_genome", help="Reference genome FASTA file"
    )
    parser.add_argument(
        "-m",
        "--mode",
...
def main():
    ...
    run_final_hard_filter(
        ...
        mismatch_model=args.mismatch_model,
