...
PADDING_BPS = 5  # 5 # 10 # revise_padding
...
MAX_ANCHOR_INDEL_BP = 150  # 20  # 12 15 18
MIN_FLANKING_SPANING_BP = 5  # 5 # 10 # revise_padding
FLANKING_ALLELE_BASE_PAIRS = 5
HMM_FLANKING_LENGTH = 5
MAX_STR_LENGTH = 150
L_STEPS_DICT = {1: 5, 2: 6, 3: 6, 4: 8, 5: 10, 6: 12}
ALLOW_MIS_OR_INS_2_F = True
...
class HMMSeggerLocus:
    ...
    def load_STR_locus_info(self, pysam_bed_row):
        ...
        ).upper()  # TODO: Inflict with init function
...
class SegRead:
    ...
    def extract_segmentation_seq(self, read):
        ...
        if self.read_spanning and self.enoughSTR:
            ...
            if (flanking_left_segmentation_site_index - self.alignment_start_index + 1) >= 20:
                left_padding_bps = 20
            elif (flanking_left_segmentation_site_index - self.alignment_start_index + 1) >= 15:
                left_padding_bps = 15
            elif (flanking_left_segmentation_site_index - self.alignment_start_index + 1) >= 10:
                left_padding_bps = 10
            else:
                left_padding_bps = (flanking_left_segmentation_site_index - self.alignment_start_index + 1)
            if (self.alignment_end_index - flanking_right_segmentation_site_index + self.alignment_start_index) >= 20:
                right_padding_bps = 20
            elif (self.alignment_end_index - flanking_right_segmentation_site_index + self.alignment_start_index) >= 15:
                right_padding_bps = 15
            elif (self.alignment_end_index - flanking_right_segmentation_site_index + self.alignment_start_index) >= 10:
                right_padding_bps = 10
            else:
                right_padding_bps = (self.alignment_end_index - flanking_right_segmentation_site_index + self.alignment_start_index)
