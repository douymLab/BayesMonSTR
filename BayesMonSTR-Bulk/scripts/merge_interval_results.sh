...
cat "$INPUT_DIR"/* | grep -v '^#' >> "$OUTPUT_DIR/${PREFIX}.txt"

# # 去除文件中的重复行，并保存到没有头的文件
sort -u -k3,3 "$OUTPUT_DIR/${PREFIX}.txt" > "$OUTPUT_DIR/${PREFIX}_no_header.vcf"

# # 按照位置对没有头的 VCF 文件进行排序
sort -k1,1 -k2,2n "$OUTPUT_DIR/${PREFIX}_no_header.vcf" > "$OUTPUT_DIR/${PREFIX}_no_header_sorted.vcf"

FIRST_FILE=$(ls "$INPUT_DIR" | head -n 1)
FIRST_FILE_PATH="$INPUT_DIR/$FIRST_FILE"
# 提取输入目录中的第一个文件的头部信息（假设所有文件的头部相同）
grep ^# ${FIRST_FILE_PATH} > "$OUTPUT_DIR/${PREFIX}_header.vcf"

# 将头文件复制到最终的排序文件
cp "$OUTPUT_DIR/${PREFIX}_header.vcf" "$OUTPUT_DIR/${PREFIX}_header_sorted.vcf"

# 将排序后的无头文件添加到头文件后面
cat "$OUTPUT_DIR/${PREFIX}_no_header_sorted.vcf" >> "$OUTPUT_DIR/${PREFIX}_header_sorted.vcf"
